...
class imshow_mark_3d:
    def __init__(self,ims,ims_c=[],fig=None,image_names=None,rescz=1.,
        ...
        self.parameters = _default_imshow_parameters.update(parameters)
        #define extra vars
        ...
        if event.button==3:
            ...
            if event.inaxes is self.ax1:
                if self.delete_mode:
                    ...
                    x_,y_,z_ = np.array(self.draw_x),np.array(self.draw_y),np.array(self.draw_z)
    ...
    def populate_draw_xyz(self,flip=False):
        if len(self.coords)>0: 
            ...
            if flip: self.draw_x,self.draw_y,self.draw_z =  list(self.draw_y),list(self.draw_x),list(self.draw_z)
            else: self.draw_x,self.draw_y,self.draw_z =  list(self.draw_x),list(self.draw_y),list(self.draw_z)
        else: 
            self.draw_x,self.draw_y,self.draw_z = [],[],[]
    ...
    def fit_seed_points(self):
        ...
        im = self.im_sm
        z_min,z_max,x_min,x_max,y_min,y_max = self.get_limits()
        y_,x_,z_ = np.array(self.draw_x),np.array(self.draw_y),np.array(self.draw_z)
        keep_class = np.array(self.class_ids)==self.index_im
