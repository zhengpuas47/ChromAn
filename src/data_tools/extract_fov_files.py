    ...
    if generate_slurm:
        ...
        if not os.path.exists(archiving_slurm_script_file) or overwrite:
            ...
            with open(archiving_slurm_script_file, 'w', encoding='utf-8') as _sf:
                ...
                _sf.write(f"#SBATCH -p {_partition}"+'\n')
                _sf.write(F"#SBATCH -A {_account}"+'\n')
                ...
                for _fov in fov_2_archive_savefile:
                    ...
                    # write line
                    _sf.write(f'sbatch -p {_partition} -A {_account} -c 1 --mem 4000 -t 0-6:00 --wrap="time tar --use-compress-program zstd -C {source_folder} -T {_filelist_savefile} -cvf {_archive_savefile}" -o {(log_folder)}/archiving_fov_{_fov}.log\n')
                    _sf.write('sleep 0.1\n')
        ...
        if not os.path.exists(scanning_slurm_script_file) or overwrite:
            ...
            with open(scanning_slurm_script_file, 'w', encoding='utf-8') as _sf:
                ...
                for _fov, _log_savefile in fov_2_log_savefile.items():
                    ...
                    _sf.write(f'sbatch -p {_partition} -A {_account} -c 1 --mem 4000 -t 0-6:00 --wrap="time tar --use-compress-program=unzstd -tf {_archive_savefile} > {_log_savefile}" -o {(log_folder)}/scanning_fov_{_fov}.log\n')
                    _sf.write('sleep 0.1\n')
        ...
        if not os.path.exists(cleanning_slurm_script_file) or overwrite:
            ...
            with open(cleanning_slurm_script_file, 'w', encoding='utf-8') as _sf:
                ...
                # copy the check_archives.py
                copyfile(os.path.join(os.path.dirname(__file__), 'check_archives.py'), os.path.join(final_target_folder, 'check_archives.py'))
        ...
        print(f'sbatch {archiving_slurm_script_file} -p {_partition} -A {_account} -t 0-12:00')
        print(f"2. scanning data archives:")
        print(f'sbatch {scanning_slurm_script_file} -p {_partition} -A {_account} -t 0-12:00')
        print(f"3. scanning data archives:")
        print(f'sbatch --wrap="python {final_target_folder+os.sep}check_archives.py -o {final_target_folder} -w" -p {_partition} -A {_account} -o {os.path.basename(source_folder)}_archive_summary.txt  -t 0-4:00')
        print(f"4. archive filelists and logs")
        print(f"sbatch {cleanning_slurm_script_file} -p {_partition} -A {_account} -t 0-1:00")
        ...
        print(f"3. scanning data archives:")
        print(f"python {final_target_folder+os.sep}check_archives.py -o {final_target_folder} -w")
        print(f"4. archive filelists and logs")
        print(f"bash {cleanning_bash_script_file}")
        print(f"-- check the final output! ")
